...
aggrFx <- function(pts,tns,qPos,accPos){
  ...
  for(code_i in c(1:N_sample)){
    ...
    curCode_condiData_xqQ1 <- curCode_condiData[curCode_condiData$question=="P != T2?",]
    ...
    for (x in 1:2) {
      ...
      fqs_hitRate_sum <- sum(fqs_hitRate_perConfidence,na.rm=T)
      ...
      fqs_faRate_sum <- sum(fqs_faRate_perConfidence,na.rm=T)
      df_code_i <- rbind(df_code_i,
      ...
      fqs_missRate_sum <- sum(fqs_missRate_perConfidence,na.rm=T)
      df_code_i <- rbind(df_code_i,
        data.frame(code=curCode,qType=qTypeNames[x],targetPosition=1,
          rating=c("sure","quiteSure","unsure"),
      ...
      fqs_crRate_sum <- sum(fqs_crRate_perConfidence,na.rm=T)
      df_code_i <- rbind(df_code_i,
        data.frame(code=curCode,qType=qTypeNames[x],targetPosition=2,
          rating=c("sure","quiteSure","unsure"),
      ...
      df2_code_i <- rbind(df2_code_i,
        data.frame(code=curCode,qName=qTypeNames[x],
          tns=tns,pts=pts,queriedPosition=qPos,accessoryPosition=accPos,
          qType="eq",
...
same_low_1_1 <- aggrFx(pts="Same",tns="low",qPos=1,accPos=1)
same_low_1_2 <- aggrFx(pts="Same",tns="low",qPos=1,accPos=2)
print(same_low_1_1)
